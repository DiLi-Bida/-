...
def test_linear_solution():
    ...
    u, t = solver(I=I, a=a, b=b, T=Nt*dt, dt=dt, theta=theta)
    u_e = u_exact(t)
    difference = abs(u_e - u).max()  # max deviation 最大范数，也叫 L^\infty 范数
    print(difference)
...
def test_convergence_rates():
    ...
    for theta in (0, 1, 0.5):
        ...
        for dt in dt_values:
            ...
            u_e = u_exact(t)
            e = u_e - u
            # L2 norm of the error, 会将误差的平方积分后开方，得到一个整体的(对一次完整的数值计算，将误差累积起来看)误差度量，是一个数
            E = sqrt(dt*sum(e**2))      
            E_values.append(E)
